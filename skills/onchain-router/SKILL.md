...
This public source release targets Buyer CLI `0.2.1` or newer in the `0.2.x` stable line. Check the canonical site's `/docs/installation`, `/products.json`, and the npm `latest` dist-tag for publication facts. Reading this skill does not authorize wallet setup, import, funding, unlock, policy widening, or a paid call.
